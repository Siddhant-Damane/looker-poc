    ...
    derived_table: {
      sql:
           select ss.drf_user_id,
              ...
              (CASE WHEN ((((CAST(ss.created_at_ms AS BIGINT) / 1000) >= ((DATE_PART(epoch, CONVERT_TIMEZONE('America/New_York', 'UTC', DATEADD(week,-2, DATE_TRUNC('week', DATE_TRUNC('day',CONVERT_TIMEZONE('UTC', 'America/New_York', GETDATE()))) )))::bigint))
              ...
              (CASE WHEN ((((CAST(ss.created_at_ms AS BIGINT) / 1000) >= ((DATE_PART(epoch, CONVERT_TIMEZONE('America/New_York', 'UTC', DATEADD(week,-1, DATE_TRUNC('week', DATE_TRUNC('day',CONVERT_TIMEZONE('UTC', 'America/New_York', GETDATE()))) )))::bigint)) AND
          ...
          --      group by 1)
            ...
            --  minus
              ...
              --(select present_play_user_not_in_beta.drf_user_id  AS "present_play_user_not_in_beta.drf_customer_id_1",
                ...
                FROM public.prod_stream_table as ss where ss.drf_user_id IS NOT NULL and ss.location_url like 'https://sports.drf.com/%'
                group by 1,3,4
                having
               (CAST(min(ss.created_at_ms) AS BIGINT) / 1000 <= (DATE_PART(epoch, DATEADD(week,2, DATE_TRUNC('week', DATE_TRUNC('day',CONVERT_TIMEZONE('UTC', 'America/New_York', GETDATE()))) ))::bigint))
                ;;
