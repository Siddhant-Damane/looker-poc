...
FROM public.prod_stream_table  AS play_user_count
WHERE (play_user_count.location_url LIKE '%pp-details%') AND (play_user_count.event_type = 'CLICK_ON_PRINT') AND (play_user_count.drf_user_id IS NOT NULL)
and ((((CAST(play_user_count.created_at_ms AS BIGINT) / 1000) >= ((DATE_PART(epoch, CONVERT_TIMEZONE('America/New_York', 'UTC', DATEADD(week,-7, DATE_TRUNC('week', DATE_TRUNC('day',CONVERT_TIMEZONE('UTC', 'America/New_York', GETDATE()))) )))::bigint))
AND (CAST(play_user_count.created_at_ms AS BIGINT) / 1000) < ((DATE_PART(epoch, CONVERT_TIMEZONE('America/New_York', 'UTC', DATEADD(week,7, DATEADD(week,-7, DATE_TRUNC('week', DATE_TRUNC('day',CONVERT_TIMEZONE('UTC', 'America/New_York', GETDATE()))) ) )))::bigint)))))
