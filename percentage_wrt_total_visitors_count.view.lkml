...
FROM public.prod_stream_table  AS play_user_count
WHERE (play_user_count.location_url LIKE '%pp-details%') AND (play_user_count.event_type = 'CLICK_ON_PRINT') AND (play_user_count.drf_user_id IS NOT NULL) group by 1 ) as D1table
