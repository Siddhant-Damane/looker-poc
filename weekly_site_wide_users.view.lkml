view: weekly_site_wide_users {

    derived_table: {
      sql:
           select ss.drf_user_id,
              max(DATE(CONVERT_TIMEZONE('UTC', 'America/New_York', (timestamp 'epoch' + CAST(ss.created_at_ms AS BIGINT) / 1000 * interval '1 second')))) AS "date",
          ...
          --      group by 1)
            ...
            --  minus
              ...
              --(select present_play_user_not_in_beta.drf_user_id  AS "present_play_user_not_in_beta.drf_customer_id_1",
                ...
                --group by 1))
                FROM public.prod_stream_table as ss where ss.drf_user_id IS NOT NULL and ss.user_tracking_id is not Null group by 1
